import logging
from app.scrapers.universities import get_all_scrapers, BejaiaSpecificScraper
...
def run_all_scrapers(app=None) -> int:
    ...
    # Béjaïa en priorité (chemin connu)
    try:
        bejaia = BejaiaSpecificScraper()
        count = bejaia.run(_app)
        total += count
        logger.info(f"[Runner] Béjaïa: {count} opportunités")
    except Exception as e:
        logger.error(f"[Runner] Erreur Béjaïa: {e}")

    # Toutes les universités (scraper intelligent)
